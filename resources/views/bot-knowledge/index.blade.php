<x-layout.app :title="'Robô & FAQ - MGI chat'">
    ...
    <div x-data="botKnowledgeApp" class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="mt-1 font-display text-3xl font-semibold text-[#5C1529]">Robô & FAQ</h2>
            ...
            <button type="button" @click="openTopicCreate()"
                    class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-[#8B1E3F] px-5 py-2.5 text-sm font-semibold text-white transition hover:opacity-90">
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            ...
            <ol class="grid gap-0 sm:grid-cols-2 lg:grid-cols-4">
                <li class="border-b border-slate-100 p-5 sm:border-r lg:border-b-0">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B1E3F] text-sm font-bold text-white">1</span>
                    <p class="mt-3 text-sm font-semibold text-slate-800">Cliente manda “oi”</p>
                ...
                <li class="border-b border-slate-100 p-5 sm:border-r-0 lg:border-b-0 lg:border-r">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B1E3F] text-sm font-bold text-white">2</span>
                    <p class="mt-3 text-sm font-semibold text-slate-800">Nome ou “de volta”</p>
                    <p class="mt-1 text-sm leading-relaxed text-slate-500">Em seguida o robô mostra o menu de assuntos (Parte B).</p>
                </li>
                <li class="border-b border-slate-100 p-5 sm:border-r lg:border-b-0">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B1E3F] text-sm font-bold text-white">3</span>
                    <p class="mt-3 text-sm font-semibold text-slate-800">Cliente escolhe um assunto</p>
                    <p class="mt-1 text-sm leading-relaxed text-slate-500">Ex.: “Pagamentos”. Depois o cliente pergunta o que precisa.</p>
                </li>
                <li class="p-5">
                    <span class="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#8B1E3F] text-sm font-bold text-white">4</span>
                    <p class="mt-3 text-sm font-semibold text-slate-800">Robô responde ou chama humano</p>
            ...
            <div class="border-t border-slate-100 bg-amber-50/60 px-5 py-3">
        ...
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                ...
                <p class="mt-1 text-2xl font-bold text-[#5C1529]" x-text="stats.topics"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Visíveis para o cliente</p>
                <p class="mt-1 text-2xl font-bold text-[#5C1529]" x-text="stats.active_topics"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Respostas cadastradas</p>
                <p class="mt-1 text-2xl font-bold text-[#5C1529]" x-text="stats.answers"></p>
            </div>
            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-4">
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div class="flex gap-3">
                    <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#8B1E3F] text-sm font-bold text-white">A</span>
                    <div>
                ...
                <button type="button" @click="saveAskName()" :disabled="savingAsk"
                        class="shrink-0 rounded-xl bg-[#8B1E3F] px-4 py-2.5 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50">
                    Salvar mensagens
                </button>
            </div>
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <div>
                    ...
                    <textarea x-model="askNameMessage" rows="4"
                              ...
                              class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#8B1E3F] focus:ring-2 focus:ring-[#8B1E3F]/10"></textarea>
        ...
        <section class="grid gap-4 lg:grid-cols-12">
            {{-- Lista de assuntos --}}
            <div class="rounded-2xl border border-slate-200 bg-white lg:col-span-4">
                <div class="border-b border-slate-100 px-4 py-4">
                    <div class="flex gap-3">
                        <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#8B1E3F] text-sm font-bold text-white">B</span>
                        <div>
                ...
                <div class="max-h-[28rem] overflow-y-auto p-2">
                    <template x-for="topic in topics" :key="topic.id">
                        <button type="button"
                                ...
                                :class="selectedId === topic.id ? 'bg-[#8B1E3F] text-white' : 'hover:bg-slate-50 text-slate-800'">
                    ...
                    <div x-show="!topics.length" class="px-3 py-10 text-center">
                        ...
                        <button type="button" @click="openTopicCreate()" class="mt-4 rounded-xl bg-[#8B1E3F] px-4 py-2 text-sm font-semibold text-white hover:opacity-90">Criar primeiro assunto</button>
                    </div>
                </div>
            </div>

            {{-- FAQs do assunto --}}
            <div class="rounded-2xl border border-slate-200 bg-white lg:col-span-8">
                <template x-if="!selected">
                    <div class="flex min-h-[22rem] flex-col items-center justify-center px-6 text-center">
                        <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-sm font-bold text-slate-500">C</span>
                ...
                <template x-if="selected">
                    <div>
                        <div class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
                            <div class="flex gap-3">
                                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-[#8B1E3F] text-sm font-bold text-white">C</span>
                                <div>
                                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Respostas deste assunto</p>
                                    <h3 class="mt-1 text-xl font-semibold text-[#5C1529]" x-text="selected.title"></h3>
                            ...
                            <div class="flex flex-wrap gap-2">
                                <button type="button" @click="openTopicEdit()" class="rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Editar assunto</button>
                                <button type="button" @click="removeTopic()" class="rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50">Excluir</button>
                                <button type="button" @click="openKnowledgeCreate()" x-show="!selected.transfers_to_human"
                                        class="rounded-xl bg-[#8B1E3F] px-3 py-2 text-sm font-semibold text-white hover:opacity-90">+ Nova resposta</button>
                            </div>
                        </div>

                        <div class="space-y-3 p-5" x-show="!selected.transfers_to_human">
                            <div class="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-3 text-sm text-slate-600">
                            ...
                            <template x-for="item in faqs" :key="item.id">
                                <article class="rounded-xl border border-slate-200 p-4 transition hover:border-slate-300">
                                    <div class="flex items-start justify-between gap-3">
                                        ...
                                        <div class="flex shrink-0 gap-1">
                                            <button type="button" @click="openKnowledgeEdit(item)" class="rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-500 hover:bg-slate-50 hover:text-[#8B1E3F]">Editar</button>
                                            <button type="button" @click="removeKnowledge(item)" class="rounded-lg px-2.5 py-1.5 text-xs font-semibold text-slate-500 hover:bg-red-50 hover:text-red-600">Excluir</button>
                                        </div>
                                    </div>
                                </article>
                            </template>

                            <div x-show="!faqs.length" class="rounded-xl border border-dashed border-slate-200 py-14 text-center">
                                ...
                                <button type="button" @click="openKnowledgeCreate()" class="mt-4 rounded-xl bg-[#8B1E3F] px-4 py-2 text-sm font-semibold text-white hover:opacity-90">Adicionar primeira resposta</button>
                            </div>
                        </div>

                        <div class="p-5" x-show="selected.transfers_to_human">
                            <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-sm text-amber-950">
        ...
        <template x-teleport="body">
            <div x-show="showTopicModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4" style="display: none;">
                <div class="absolute inset-0 bg-black/40" @click="closeTopicModal()"></div>
                <div class="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl" @click.stop>
                    <h3 class="text-lg font-semibold text-[#5C1529]" x-text="editingTopic ? 'Editar assunto do menu' : 'Novo assunto do menu'"></h3>
                    <p class="mt-1 text-sm text-slate-500">O título é o que o cliente vê na lista do WhatsApp.</p>
                    <div class="mt-4 space-y-3">
                        <div>
                            ...
                            <input type="text" x-model="topicForm.title" maxlength="80" placeholder="Ex.: Pagamentos, Horário, Falar com atendente"
                                   class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-[#8B1E3F]">
                        </div>
                        <div>
                        ...
                        <label class="flex items-start gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 text-sm text-slate-700">
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <button type="button" @click="closeTopicModal()" class="rounded-xl px-4 py-2 text-sm font-medium text-slate-500 hover:bg-slate-50">Cancelar</button>
                        <button type="button" @click="saveTopic()" :disabled="saving" class="rounded-xl bg-[#8B1E3F] px-4 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50">Salvar assunto</button>
                    </div>
                </div>
            </div>
        </template>
        ...
        <template x-teleport="body">
            <div x-show="showKnowledgeModal" x-cloak class="fixed inset-0 z-[80] flex items-center justify-center p-4" style="display: none;">
                <div class="absolute inset-0 bg-black/40" @click="closeKnowledgeModal()"></div>
                <div class="relative z-10 w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl" @click.stop>
                    <h3 class="text-lg font-semibold text-[#5C1529]" x-text="editingKnowledge ? 'Editar resposta' : 'Nova resposta do robô'"></h3>
                    <p class="mt-1 text-sm text-slate-500">Preencha os 3 campos: o que é a dúvida, o que o robô responde, e quais palavras disparam essa resposta.</p>
                    <div class="mt-4 space-y-3">
                        <div>
                        ...
                        <div>
                            ...
                            <textarea x-model="knowledgeForm.answer" rows="4" placeholder="Ex.: Para emitir a 2ª via, acesse o portal… ou digite seu CPF."
                                      class="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-[#8B1E3F]"></textarea>
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        <button type="button" @click="closeKnowledgeModal()" class="rounded-xl px-4 py-2 text-sm font-medium text-slate-500 hover:bg-slate-50">Cancelar</button>
                        <button type="button" @click="saveKnowledge()" :disabled="saving" class="rounded-xl bg-[#8B1E3F] px-4 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50">Salvar resposta</button>
                    </div>
                </div>
            </div>
        </template>
