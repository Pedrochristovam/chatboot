<x-layout.app :title="'Conversas - MGI chat'" :fullWidth="true">
    ...
    <div x-data="chatApp()" x-init="init()" class="flex h-[calc(100vh-4rem)] overflow-hidden lg:h-[calc(100vh-5rem)]">
        ...
        <div :class="mobileView === 'chat' || mobileView === 'info' ? 'hidden lg:flex' : 'flex'"
            ...
            <div class="border-b border-slate-200 bg-white p-4">
                <div class="mb-3 flex items-center justify-between">
                    <div>
                        <p class="font-display text-lg font-semibold text-[#5C1529]">Inbox</p>
                        <p class="text-xs text-slate-400" x-text="conversations.length + ' conversa(s)'"></p>
                    </div>
                </div>
                <div class="relative">
                ...
                <div class="mt-3 flex gap-2 overflow-x-auto pb-0.5">
                    <template x-for="f in filters" :key="f.id">
                        <button type="button" @click="activeFilter = f.id; loadConversations()"
                                ...
                                class="shrink-0 rounded-full px-3.5 py-1.5 text-xs font-semibold transition" x-text="f.label"></button>
                    </template>
                </div>
            </div>
            <div class="custom-scrollbar flex-1 overflow-y-auto bg-white">
                ...
                <div x-show="conversations.length === 0" class="flex flex-col items-center px-6 py-16 text-center">
        ...
        <div :class="mobileView !== 'chat' ? 'hidden lg:flex' : 'flex'" class="relative flex flex-1 flex-col bg-white">
            <div class="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
                <div class="flex items-center gap-3">
                    ...
                    <template x-if="selected">
                        <div class="flex items-center gap-3">
                            <div class="flex h-11 w-11 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-[#5C1529]" x-text="selected.initials"></div>
                            <div>
                ...
                <div class="flex items-center gap-2" x-show="selected && !isReadOnly">
                    <button type="button" @click="closeConversation()" class="rounded-xl bg-[#8B1E3F] px-3.5 py-2 text-sm font-semibold text-white shadow-sm hover:opacity-90">Encerrar</button>
            ...
            <div class="custom-scrollbar flex-1 space-y-3 overflow-y-auto p-4 sm:p-5" @click="showEmojiPicker = false">
                <div x-show="!selected && !loading" class="flex h-full min-h-[16rem] flex-col items-center justify-center text-center">
                    ...
                    <p class="font-display text-xl font-semibold text-[#5C1529]">MGI chat</p>
            ...
            <div class="relative border-t border-slate-200 bg-white/95 p-3 backdrop-blur-sm" x-show="selected">
                <div x-show="isReadOnly" class="mb-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-center text-sm text-[#5C1529]">
                ...
                <div class="relative" x-show="!isReadOnly" @click.outside="showEmojiPicker = false">
                    <div x-show="showEmojiPicker"
                         ...
                         class="absolute bottom-full left-0 right-0 z-50 mb-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
                        <div class="border-b border-slate-100 px-3 py-2">
                            <input type="text" x-model="emojiSearch" @input="refreshVisibleEmojis()"
                                   placeholder="Filtrar categorias (ex: Comida)"
                                   class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-[#8B1E3F]">
                        </div>
                    ...
                    <div class="flex items-end gap-2 rounded-2xl bg-white p-1.5 ring-1 ring-slate-200">
                        <button type="button"
                                ...
                                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl transition" title="Emojis">
                            <span class="text-xl leading-none">😊</span>
                        </button>
                        <label class="flex h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-xl text-slate-500 transition hover:bg-white hover:text-[#8B1E3F]" title="Enviar foto">
                        ...
                        <button type="button" @click="sendMessage()" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-[#8B1E3F] text-white transition hover:opacity-90">
        ...
        <div :class="mobileView !== 'info' ? 'hidden xl:flex' : 'flex'" class="w-full flex-col border-l border-slate-200 bg-white xl:w-80">
            <div class="custom-scrollbar flex-1 overflow-y-auto" x-show="selectedClient">
                <div class="bg-white px-5 pb-6 pt-8 text-center">
                    ...
                    <h3 class="font-display text-xl font-semibold text-[#5C1529]" x-text="selectedClient?.name"></h3>
                ...
                <div class="space-y-4 p-5">
                    <div>
                        ...
                        <div class="rounded-2xl bg-white px-4 py-3 text-sm text-slate-700 ring-1 ring-slate-200" x-text="selectedClient?.company || 'Não informado'"></div>
                    </div>
                    <div>
                        <h4 class="mb-2 text-xs font-bold uppercase tracking-wider text-slate-400">Notas do Cliente</h4>
                        <div class="rounded-2xl bg-white px-4 py-3 text-sm leading-relaxed text-slate-700 ring-1 ring-slate-200" x-text="selectedClient?.notes || 'Sem observações.'"></div>
                    </div>
                </div>
            </div>
